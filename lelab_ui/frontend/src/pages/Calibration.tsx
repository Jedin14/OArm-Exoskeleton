  ...
  const handleStartCalibration = async () => {
    ...
    if (port.startsWith("openarm_ros") || port.startsWith("ROS2 (lyrical)")) {
      try {
        ...
        toast({
          title: "Saved",
          description: "ROS 2 Bridge configured! No hardware calibration needed.",
        });
        fetchRobot();
        // Also simulate completion so the UI advances to the next device
        setCalibrationStatus((prev) => ({ ...prev, status: "completed" }));
      } catch (e) {
        console.error("Failed to save ROS 2 Bridge config:", e);
      }
      return;
    }
