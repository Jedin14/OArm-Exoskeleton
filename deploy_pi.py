...
Usage:
    python deploy_pi.py                 # Both arms (default)
    python deploy_pi.py --right         # Right arm only
    python deploy_pi.py --model /path/to/model
"""
...
# ── Configuration ─────────────────────────────────────────────────────────────
MODEL_PATH = "/home/jed/openarm_models/pi0.5"
RIGHT_CAN  = "can0"
LEFT_CAN   = "can1"
...
def inference_worker(policy, cam_main, cam_right, cam_left, is_bimanual, stats, task_str):
    ...
    tokenized = tokenizer([task_str + "\n"], padding="max_length", max_length=200, return_tensors="pt")
    lang_tokens = tokenized["input_ids"].to(DEVICE)
    lang_masks = tokenized["attention_mask"].to(DEVICE)
...
def main():
    ...
    parser.add_argument("--8gb_vram", action="store_true",
                        help="Run with 8GB VRAM optimizations (offload vision to CPU)")
    parser.add_argument("--task", required=True, type=str,
                        help="Language instruction for the robot (e.g. 'pick a packet from box 1 and place it in box 2')")
    ...
    print(f"Loading model from {args.model} in bfloat16 to fit in VRAM...")
    torch.set_default_dtype(torch.bfloat16)
    
    if getattr(args, "8gb_vram", False):
        from lerobot.configs.policies import PreTrainedConfig
        print("Using 8GB VRAM mode: loading to CPU first, then applying offload...")
        config = PreTrainedConfig.from_pretrained(args.model)
        config.device = "cpu"
        policy = PI05Policy.from_pretrained(args.model, config=config)
        
        print("Manually patching embed_image and embed_language_tokens for offloading...")
        # Keep vision encoder on CPU, only move to CUDA during embedding
        original_embed_image = policy.model.paligemma_with_expert.embed_image
        def new_embed_image(img):
            policy.model.paligemma_with_expert.paligemma.to('cuda')
            res = original_embed_image(img)
            policy.model.paligemma_with_expert.paligemma.to('cpu')
            torch.cuda.empty_cache()
            return res
        policy.model.paligemma_with_expert.embed_image = new_embed_image
        
        original_embed_lang = policy.model.paligemma_with_expert.embed_language_tokens
        def new_embed_lang(tokens):
            policy.model.paligemma_with_expert.paligemma.to('cuda')
            res = original_embed_lang(tokens)
            policy.model.paligemma_with_expert.paligemma.to('cpu')
            torch.cuda.empty_cache()
            return res
        policy.model.paligemma_with_expert.embed_language_tokens = new_embed_lang
        
        # Keep action expert on CUDA permanently
        policy.model.paligemma_with_expert.gemma_expert.to('cuda')
        policy.model.action_in_proj.to('cuda')
        policy.model.action_out_proj.to('cuda')
        policy.model.time_mlp_in.to('cuda')
        policy.model.time_mlp_out.to('cuda')
        print("Successfully applied manual offload hooks!")
    else:
        policy = PI05Policy.from_pretrained(args.model)
        policy.to(DEVICE)
    ...
    inference_thread = threading.Thread(target=inference_worker, 
                                  args=(policy, cam_main, cam_right, cam_left, is_bimanual, stats, args.task), 
                                  daemon=True, name="inference_worker")
