...
void MotorControl::recv_set_param_data() {
  ...
  // Expected format (assumed):
  // data[0]: SlaveID low, data[1]: SlaveID high
  // data[2]: cmd echo (e.g., 0x33 read), data[3]: RID
  // data[4..7]: value (uint32)
  if (len >= 8) {
    uint16_t slave_id = static_cast<uint16_t>(data[0]) |
                        (static_cast<uint16_t>(data[1]) << 8);
    uint8_t rid = data[3];

    // Parse as uint32 value
    uint32_t value = uint8s_to_uint32(data[4], data[5], data[6], data[7]);

    auto it = motors_map_.find(slave_id);
    if (it != motors_map_.end() && it->second) {
      it->second->setTempParam(static_cast<int>(rid), static_cast<int>(value));
    }
  }
}
