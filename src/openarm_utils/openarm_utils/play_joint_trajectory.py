...
class TrajectoryPlayer(Node):
    ...
    async def play_all_joints_async(self, joint_names: List[str], points: List[Dict], rate_scale: float = 1.0) -> None:
        ...
        left_arm_joints = [j for j in joint_names if j.startswith('openarm_left_joint')]
        right_arm_joints = [j for j in joint_names if j.startswith('openarm_right_joint')]
        left_gripper_joints = [j for j in joint_names if j.startswith('openarm_left_finger')]
        right_gripper_joints = [j for j in joint_names if j.startswith('openarm_right_finger')]
...
def main() -> None:
    ...
    if args.joints:
        ...
        target_joints = [f'openarm_left_joint{i}' for i in range(1, 8)]
    elif args.right_arm:
        target_joints = [f'openarm_right_joint{i}' for i in range(1, 8)]
    elif args.both_arms:
        target_joints = [f'openarm_left_joint{i}' for i in range(1, 8)] + [f'openarm_right_joint{i}' for i in range(1, 8)]
