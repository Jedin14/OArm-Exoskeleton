...
# ---------- Global State for Active Hold ----------
TARGET_GRIP_POS = 0.0
ARM_PARAMS = None

def active_sleep(openarm, duration):
    if ARM_PARAMS is None:
        time.sleep(duration)
        return
    t_end = time.time() + duration
    while time.time() < t_end:
        openarm.get_arm().mit_control_all(ARM_PARAMS)
        openarm.get_gripper().mit_control_all([oa.MITParam(10.0, 0.9, TARGET_GRIP_POS, 0.0, 0.0)])
        openarm.recv_all()
        time.sleep(0.01)

# ---------- IDs / Limits / Signs ----------
class JointID(IntEnum):
    J1 = 0; J2 = 1; J3 = 2; J4 = 3; J5 = 4; J6 = 5; J7 = 6; GRIPPER = 0
...
def interpolate(openarm, comp, joint_id: JointID, delta_rad, kp=52.0, kd=1.5, torque_assist=0.0, interp_time=2.0):
    idx = int(joint_id)
    ...
    for i in range(n_steps + 1):
        alpha = i / n_steps
        q = q0 + (q1 - q0) * alpha
        ARM_PARAMS[idx] = oa.MITParam(kp, kd, q, 0.0, tau)
        active_sleep(openarm, dt)
...
def bump_to_limit(openarm, comp, joint_id, step_deg=0.2, kp=45.0, kd=1.2, torque_bias=0.0):
    idx = int(joint_id)
    ...
    while True:
        q_target += step_rad
        ARM_PARAMS[idx] = oa.MITParam(kp, kd, q_target, 0.0, tau_cmd)
        active_sleep(openarm, 0.005)
        ...
        if np.abs(m.get_velocity()) < dq_th and np.abs(m.get_torque()) > tau_th:
            ...
            print(f"[INFO] mechanical stop (Joint {joint_id.name}): {delta_rad:.4f} rad / {delta_deg:.2f}°")
...
def _run_right_sequence(openarm, arm, grip):
    ...
    interpolate(openarm, arm, JointID.J4, -np.pi/2.0)
    active_sleep(openarm, 2.5)

    ideal = {
        JointID.J7: np.pi/2.0, JointID.J6: np.pi/4.0, JointID.J5: np.pi/2.0, JointID.J4: 0.0,
        JointID.J3: np.pi/2.0, JointID.J2: np.deg2rad(-10), JointID.J1: np.deg2rad(-80),
    }
...
def configure_can_interface(interface):
    ...
    subprocess.run(["sudo", "ip", "link", "set", interface, "type", "can", "bitrate", "1000000", "dbitrate", "5000000", "fd", "on"], check=True)
    subprocess.run(["sudo", "ip", "link", "set", interface, "up"], check=True)

def calibrate_arm(canport, arm_side):
    global TARGET_GRIP_POS, ARM_PARAMS
    print(f"\n========================================")
    print(f"Starting {arm_side.upper()} calibration on {canport}...")

    yaml_file = 'gripper_home.yaml'
    if not os.path.exists(yaml_file):
        print(f"ERROR: '{yaml_file}' not found. Run save_gripper_home.py first.", file=sys.stderr)
        sys.exit(1)
    with open(yaml_file, 'r') as f:
        data = yaml.safe_load(f)
        if arm_side == 'right_arm':
            TARGET_GRIP_POS = data['gripper_home']['right_arm_can0']
        else:
            TARGET_GRIP_POS = data['gripper_home']['left_arm_can1']
    ...
    print(f"Moving gripper smoothly to saved position {TARGET_GRIP_POS:.3f} rad...")
    for step in range(1, 21):
        current_kp = 10.0 * (step / 20.0)
        mit = oa.MITParam(current_kp, 0.5, TARGET_GRIP_POS, 0.0, 0.0)
        openarm.get_gripper().mit_control_all([mit])
        openarm.recv_all()
        time.sleep(0.05)
    ...
    grip.mit_control_all([oa.MITParam(10.0, 0.9, initial_grip_q[0], 0.0, 0.0)])
    openarm.recv_all()

    try:
        ideal, deltas = (_run_right_sequence if arm_side == 'right_arm' else _run_left_sequence)(openarm, arm, grip)

        joint_order = [JointID.J1, JointID.J2, JointID.J3, JointID.J4, JointID.J5, JointID.J6, JointID.J7, JointID.GRIPPER]
        ideal_delta_map = {}
        for j, d in zip(joint_order, deltas):
            ...
            val = calc_delta_to_zero_pos_joint(
                initial_rad=initial_arm_q[int(j)], ideal_limit_rad=ideal[j],
                delta_to_stop_rad=d, joint_id=j
            )
...
def main():
    ...
    calibrate_arm("can0", "right_arm")
    calibrate_arm("can1", "left_arm")
